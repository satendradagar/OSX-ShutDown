...
+ (BOOL) runAppleScript:(NSString*)scriptName
     withResponseHandling:(void (^)(NSString *message, NSError *error))handler {
    
    NSString  * scriptPath = [TaskManager resouceURLForAppleScript:scriptName];
    NSString *errorDescription = nil;
    NSString *output = nil;
    NSMutableDictionary *errorInfo = [NSMutableDictionary new];

    NSAppleScript *appleScript = [[NSAppleScript new] initWithContentsOfURL:[NSURL fileURLWithPath:scriptPath] error:&errorInfo];
    NSAppleEventDescriptor * eventResult = [appleScript executeAndReturnError:&errorInfo];
...
+(NSString *)resouceURLForAppleScript:(NSString *)scriptName{
    
    NSString  * scriptPath = [[NSBundle bundleForClass:[self class]] pathForResource:scriptName ofType:@"scpt"];
    return scriptPath;
}
